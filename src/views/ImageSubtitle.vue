  ...
  <div class="isub-layout">
    ...
    <div class="isub-left">
      ...
      <div v-if="images.length > 0" class="info-bar">
        ...
        <span><i class="fa-solid fa-expand"></i> {{ images[currentIndex]?.width }} × {{ images[currentIndex]?.height }}</span>
    ...
    <div class="isub-right">
      ...
      <div class="hint-box">
        <div class="hint-line">
          <span class="line-dot" style="background:#ef4444"></span>
          <span><strong>红线</strong>（上边界）：后续图片从此处开始保留</span>
        </div>
        <div class="hint-line">
          <span class="line-dot" style="background:#3b82f6"></span>
          <span><strong>蓝线</strong>（下边界）：所有图片保留到此处结束</span>
        </div>
        <div class="hint-line" style="color:var(--muted); font-size:0.75rem">
          <i class="fa-solid fa-hand-pointer"></i>
          <span>鼠标拖拽线条调整，实时预览</span>
        </div>
      </div>
...
.canvas-stage {
  ...
  overflow: hidden;      /* 防止 Canvas 超出圆角 */
  display: flex;
  align-items: center;
...
.info-bar i { color: var(--accent); margin-right: 0.25rem; }
...
@media (max-width: 640px) {
  .isub-layout { flex-direction: column; }
  .isub-right  { width: 100%; }
}
</style>